 ...
 *
 * when headset is pluged in, we determine whether to standby output stream
 * according if SRP was running and next output device.
 */ 
#ifdef USE_ULP_AUDIO
#define SRP_CTRL "/dev/srp_ctrl"
...
#define VOICE_RECORD_OPEN 0
#define VOICE_RECORD_START 1
#define VOICE_RECORD_CLOSE 2
...
AudioHardware::AudioHardware() :
    ...
    mATChannelOk(-1)
{
    //Modified by CYIT 20130730 for voice memo----start
    //loadRILD();
#if 0   //wjj close temp.
    mSecRil = SecRil::instance(); // Modified by CYIT 20130818
#else
    mSecRil = NULL;
#endif

    //Create a new thread for ensuring that the AT channel would be opened OK.
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
    pthread_create(&mOpenATInterfaceThread, &attr, ThreadWrapper, this);
    pthread_attr_destroy(&attr);
    //Modified by CYIT 20130730 for voice memo----end
    mInit = true;
}
...
AudioHardware::~AudioHardware()
{
    ...
    //Modified by CYIT 20130730 for voice memo----start
    void *dummy;
    pthread_join(mOpenATInterfaceThread, &dummy);

    status_t err = (status_t) dummy;
    LOGE("join mOpenATInterfaceThread err = %d", err);

    mATChannelOk = -1;
    //Modified by CYIT 20130730 for voice memo----end

    if (mSecRil != NULL) {
        mSecRil->closeAudioATChannel();
    } else {
        LOGE("closeAudioATChannel mSecRil == NULL!!!");
    }
...
//Add by CYIT 20130730 for voice memo----start
//static
void *AudioHardware::ThreadWrapper(void *me) {
    ALOGV("ThreadWrapper: %p", me);
    AudioHardware *hw = static_cast<AudioHardware *>(me);
    hw->threadFunc();
    return NULL;
}

void AudioHardware::threadFunc()
{
    if (mSecRil != NULL) {
        mATChannelOk = mSecRil->openAudioATChannel();
        LOGE("for audio only, mATChannelOk = %d", mATChannelOk);
    } else {
        LOGE("mSecRil is NULL!!!");
    }
}
//Add by CYIT 20130730 for voice memo----end

void AudioHardware::loadRILD(void)
{
    // modified by CYIT
    mSecRilLibHandle = dlopen("/system/lib/libsecril-client.so", RTLD_NOW);
    ...
    if (mSecRilLibHandle) {
        ...
        // modified by CYIT
        setCallVmOperation = (int (*)(HRilClient, int))
                              dlsym(mSecRilLibHandle, "setCallVmOperation");
        // modified by CYIT
        if (!openClientRILD  || /*!disconnectRILD   ||*/ !closeClientRILD ||
            /*!isConnectedRILD ||*/ !connectRILD      ||
            !setCallVolume   || !setCallAudioPath /*|| !setCallClockSync*/||!setCallVmOperation) {
...
status_t AudioHardware::setMode(int mode)
{
    ...
    LOGD("setMode() : new %d, old %d", mMode, prevMode);
    if (status == NO_ERROR) {
...
// modified by CYIT
        ...
        if (mMode != AudioSystem::MODE_IN_CALL && mInCallAudioMode) {
            ...
            if (mMixer != NULL) {
                ...
                const AudioMixer *mixer = device_voice_OFF;
                for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
                    ...
                    if (NULL != ctl) {
                        ...
                        TRACE_DRIVER_OUT
                        LOGE("exit incall %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
                    } else {
                        LOGE("555failed get mixer control");
                    }
                }
	    }
...
#if 0 // [mpappas] loop path for factory test
    key = String8("looptest");
    if (param.get(key, value) == NO_ERROR) {
        uint32_t device;
        if (value == "handset") {
            device = AudioSystem::DEVICE_OUT_LOOP_HANDSET;
        } else if (value == "headset") {
            device = AudioSystem::DEVICE_OUT_LOOP_HEADSET;
        } else if (value == "0") {
		...
		if((device == AudioSystem::DEVICE_OUT_LOOP_HEADSET) || 
					(device == AudioSystem::DEVICE_OUT_LOOP_HANDSET)){
	        struct mixer_ctl *ctl;
	        const AudioMixer *mixer = getOutputRouteFromDevice(device);;
	        for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
	            TRACE_DRIVER_IN(DRV_MIXER_GET)
	            ctl = mixer_get_ctl_by_name(mMixer, mixer[cnt].ctl);
	            TRACE_DRIVER_OUT
	            if (NULL != ctl) {
	                ...
	                for (unsigned int idx = 0 ; mixer_ctl_get_num_values(ctl) > idx ; ++idx)
	                    mixer_ctl_set_value(ctl, idx, mixer[cnt].val);
	                TRACE_DRIVER_OUT
	                LOGE("enter looptest setting %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
	            } else {
	                LOGE("failed get mixer control");
	            }
	        }
		}
    }
...
String8 AudioHardware::getParameters(const String8& keys)
{
    ...
    LOGV("getParameters() %s, mMode: %d", keys.string(), mMode);

    if (mMode ==  AudioSystem::MODE_IN_CALL) {
	request.addInt(String8("AMR"), true);
...
{
    if ((format != AudioSystem::PCM_16_BIT) && (format != AudioSystem::AMR_NB)) {
...
#if 1
    mVoiceVol = volume;
LOGE("setVoiceVolume_l mMode = %d", mMode);
    // modified by CYIT
...
//         (connectRILDIfRequired() == OK)*/ ) {
    if (AudioSystem::MODE_IN_CALL == mMode) {
        ...
        if (mSecRil != NULL && mATChannelOk ==0) {
            mSecRil->setCallVolume(type, int_volume);
        }else {
            LOGE("setCallVolume mSecRil == %p, mATChannelOk = %d ERROR !!!", mSecRil.get(), mATChannelOk);
        }
        //Modified by CYIT 20130730 for voice memo----end
    }
#else
	mVoiceVol = volume;
        if(AudioSystem::MODE_IN_CALL == mMode){
            ...
            case AudioSystem::DEVICE_OUT_EARPIECE:
				LOGD("### earpiece call volume");
                ctlname = "Output Volume";
                break;
				
            case AudioSystem::DEVICE_OUT_WIRED_HEADSET:
            case AudioSystem::DEVICE_OUT_WIRED_HEADPHONE: // Use receive path with 3 pole headset.
                LOGD("### headset call volume");
                ctlname = "Headphone Volume";
                break;

            case AudioSystem::DEVICE_OUT_SPEAKER:
                LOGD("### speaker call volume");
                ctlname = "Speaker Volume";
                break;
            ...
            if (NULL != ctlname) {
                TRACE_DRIVER_IN(DRV_MIXER_GET)
                ctl = mixer_get_ctl_by_name(mMixer, ctlname);
                TRACE_DRIVER_OUT
                if (NULL == ctl) {
                    LOGE("failed get mixer control");
                    return;
                ...
                for (unsigned int i = 0; i < num_values; i++) {
                    if (mixer_ctl_set_percent(ctl, i, int_volume)) {
                        LOGW("Error: invalid value\n");
                        return;
                    }
                }
                LOGW("### Call volume setting ok!!!volume =%d\n", int_volume);
                TRACE_DRIVER_OUT
            }
    }
#endif
...
int AudioHardware::setCallVmOperation_l(int order)
{

//Modified by CYIT 20130730 for voice memo----start
#if 0
    if (setCallVmOperation(mRilClient, order) != 0) {
        LOGV("setCallVmOperation err");
        return UNKNOWN_TRANSACTION;
    }
#endif
    if (mSecRil != NULL && mATChannelOk == 0) {
        if (mSecRil->setCallVmOperation(order) < 0) {
            return UNKNOWN_TRANSACTION;
        }
    } else {
        LOGE("setCallVmOperation mSecRil == %p, mATChannelOk = %d ERROR !!!", mSecRil.get(), mATChannelOk);
        return UNKNOWN_TRANSACTION;
    }
    return NO_ERROR;
//Modified by CYIT 20130730 for voice memo----end
...
{
    LOGD("setIncallPath_l: device %x", device);
    if (mVoiceDevice == device)
        return NO_ERROR;
...
// modified by CYIT
    ...
    if (1){
        ...
        if (mMode == AudioSystem::MODE_IN_CALL) {
            ...
            if (mSecRil != NULL && mATChannelOk == 0) {
                mSecRil->setCallAudioPath(path);
            } else {
                LOGE("setCallAudioPath mSecRil == %p, mATChannelOk = %d ERROR!!!", mSecRil.get(), mATChannelOk);
            }
            //Modified by CYIT 20130730 for voice memo----end

            if (NULL == mMixer) {
                LOGE("failed openMixer");
                return NO_INIT;
            }
#endif
            struct mixer_ctl *ctl;
            ...
            for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
                ...
                if (NULL != ctl) {
                    ...
                    for (unsigned int idx = 0 ; mixer_ctl_get_num_values(ctl) > idx ; ++idx)
                        mixer_ctl_set_value(ctl, idx, mixer[cnt].val);
                    TRACE_DRIVER_OUT
                    LOGD("setting incall path %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
                } else {
                    LOGE("4444failed get mixer control");
                }
            }
        }

        setVoiceVolume_l(mVoiceVol);//chy add@20111209,when change the output device,then set the previous volume again.
    }
    return NO_ERROR;

}

struct pcm *AudioHardware::openVoicePcmOut_l()
{
    LOGD("openVoicePcmOut_l(): mVoicePcmOpenCnt(%d)", mVoicePcmOpenCnt);
    if (mVoicePcmOpenCnt++ == 0) {
        if (mVoicePcm != NULL) {
            LOGE("openVoicePcmOut_l() mVoicePcmOpenCnt = 0, mVoicePcm == %p\n", mVoicePcm);
            mVoicePcmOpenCnt--;
            return NULL;
        }
        unsigned flags = PCM_OUT;

        // wenpin.cui: just make sure pcm_open success, driver will handle
        // incall specific settings.
        ...
        mVoicePcm = pcm_open(0, 2, flags, &config); // wenpin.cui: card 0, device 2
        TRACE_DRIVER_OUT

        if (!pcm_is_ready(mVoicePcm)) {
            LOGE("openVoicePcmOut_l() cannot open pcm_out driver: %s\n",
    	     pcm_get_error(mVoicePcm));
            TRACE_DRIVER_IN(DRV_PCM_CLOSE)
            pcm_close(mVoicePcm);
            TRACE_DRIVER_OUT
            mVoicePcm = NULL;
            ...
            LOGD("starting voice call pcm stream ...");
            pcm_start(mVoicePcm);
        }
    }
...
void AudioHardware::closeVoicePcmOut_l()
{
    LOGD("closeVoicePcmOut_l() mVoicePcmOpenCnt: %d", mVoicePcmOpenCnt);
    mVoiceDevice = 0;
    if (mVoicePcmOpenCnt == 0) {
        LOGE("closeVoicePcmOut_l() mPcmOpenCnt == 0");
        return;
    }

    if (--mVoicePcmOpenCnt == 0) {
        if (NULL == mVoicePcm) {
            LOGE("closeVoicePcmOut_l() mPcmOpenCnt != 0, mVoicePcm = NULL");
            return;
        }
...
#else //delete duplicated item:wenpin.cui
    switch (device) {
    case AudioSystem::DEVICE_OUT_EARPIECE:
    return device_out_RCV; //PC4
    case AudioSystem::DEVICE_OUT_SPEAKER:
        return device_out_SPK;
    case AudioSystem::DEVICE_OUT_WIRED_HEADPHONE:
    case AudioSystem::DEVICE_OUT_WIRED_HEADSET:
        return device_out_HP;
    case (AudioSystem::DEVICE_OUT_SPEAKER|AudioSystem::DEVICE_OUT_WIRED_HEADPHONE):
    case (AudioSystem::DEVICE_OUT_SPEAKER|AudioSystem::DEVICE_OUT_WIRED_HEADSET):
        return device_out_SPK_HP;
    case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO:
    case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_HEADSET:
    case AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_CARKIT:
        return device_out_BT;
#if 0 // [mpappas] loop path for factory test
    case AudioSystem::DEVICE_OUT_LOOP_HANDSET:
        return device_out_LOOP_Handset;
    case AudioSystem::DEVICE_OUT_LOOP_HEADSET:
        return device_out_LOOP_Headset;
#endif
    default:
        return device_out_OFF;
    }
...
const AudioMixer *AudioHardware::getVoiceRouteFromDevice(uint32_t device)
{
    ...
    switch (device) {
    case AudioSystem::DEVICE_OUT_EARPIECE:
//        return device_voice_RCV; //we don't have RCV
    case AudioSystem::DEVICE_OUT_SPEAKER:
...
#else //wenpin.cui: remove duplicated item
    switch (device) {
    case AudioSystem::DEVICE_OUT_EARPIECE:
         ...
         if ((source == AUDIO_SOURCE_DEFAULT) || (mMode != AudioSystem::MODE_IN_CALL)) {
             if (mMixer) {
                 ...
                 if (NULL != ctl) {
                     ...
                     LOGE("3333failed get mixer control");
                 }
             }
         }
         mInputSource = source;
...
AudioHardware::AudioStreamOutALSA::AudioStreamOutALSA() :
    ...
    mStandby(true), mBtScoOut(false), mDevices(0), mChannels(AUDIO_HW_OUT_CHANNELS),
...
{
    LOGV("-----AudioStreamOutALSA::write(%p, %d) START", buffer, (int)bytes);
    ...
    /*
     * wenpin.cui:
     * If inputstream became active, it will require hw/in/out lock, close pcm_out and reopen it.
     * If we get called during this period, capture will become abnormal.
     * So we require hw lock before requiring instream lock.
     * If we succeed, inputstream must have finished his stuff, it's ok to go ahead.
     */
    mHardware->lock().lock();
    ...
    { // scope for the lock
        ...
        if (mStandby) {
            ...
            while (spIn != 0) {
                ...
                if ((spIn == mHardware->getActiveInput_l()) &&
                        (cnt == spIn->standbyCnt())) {
                    LOGD("AudioStreamOutALSA::write() force input standby");
                    spIn->close_l();
                    break;
                }
                spIn->unlock();
        ...
        if (ret == 0) {
            LOGV("-----AudioStreamOutALSA::write(%p, %d) END", buffer, (int)bytes);
            return bytes;
        }
...
void AudioHardware::AudioStreamOutALSA::doStandby_l()
{
	...
	// wenpin.cui: decrease noise when sound card goes to standby.
	if (!mMixer) {
	...
	if (mHardware->mode() != AudioSystem::MODE_IN_CALL) {
	    const AudioMixer *mixer = device_out_OFF;
	    for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt) {
		TRACE_DRIVER_IN(DRV_MIXER_GET)
		...
		if (NULL != mRouteCtl) {
		    ...
		    TRACE_DRIVER_OUT
			LOGD("standby() setting %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
		} else
		    LOGE("666failed get mixer control");
	    }
	}
...
void AudioHardware::AudioStreamOutALSA::close_l()
{
    ...
    if (mBtScoOut) {
        LOGD("close_l: stop bt sco related pcm");
        mBtScoOut = false;
        mHardware->closeVoicePcmOut_l();
    }
}
...
status_t AudioHardware::AudioStreamOutALSA::open_l()
{
    ...
    // start bt sco related pcm.
    if (mHardware->mode() != AudioSystem::MODE_IN_CALL && (!mBtScoOut) &&
        (mDevices & AUDIO_DEVICE_OUT_ALL_SCO)) {
        LOGD("open_l: start bt sco related pcm");
        mHardware->openVoicePcmOut_l();
        mBtScoOut = true;
    }
        ...
        for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
            ...
            if (NULL != mRouteCtl) {
                ...
                for (unsigned int idx = 0 ; mixer_ctl_get_num_values(mRouteCtl) > idx ; ++idx)
                    mixer_ctl_set_value(mRouteCtl, idx, mixer[cnt].val);
                TRACE_DRIVER_OUT
                LOGD("write() wakeup setting %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
            } else {
                LOGE("2222failed get mixer control");
            }
        }
    }
    return NO_ERROR;
...
AudioHardware::AudioStreamInALSA::AudioStreamInALSA() :
    mHardware(0), mPcm(0), mMixer(0), mRouteCtl(0), mFd(-1), // mFd ported from cyit
    mStandby(true), mBtScoIn(false), mDevices(0), mChannels(AUDIO_HW_IN_CHANNELS), mChannelCount(1),
...
{
    if (pFormat == 0 || ((*pFormat != AUDIO_HW_IN_FORMAT) && (*pFormat != AudioSystem::AMR_NB))) {
    ...
    if (pChannels == 0 ||
	(*pChannels & (AudioSystem::CHANNEL_IN_MONO | AudioSystem::CHANNEL_IN_STEREO)) == 0) {
	...
	LOGV("AudioHardware::AudioStreamInALSA::set format AMR");

	//use ril tell the cp open
	LOGV("setCallVmOperation_l VOICE_RECORD_OPEN");
	if (mHardware->setCallVmOperation_l(VOICE_RECORD_OPEN)) {
	    LOGE("setCallVmOperation_l VOICE_RECORD_OPEN Error ");
	    return -EPERM;
	}
	// open vocie memo
    char property[PROPERTY_VALUE_MAX];
    property_get("gsm0710mux.channel19", property, "");
    if (mFd >= 0) {
        LOGE("Audio record already open");
    } else {
        //LOGD("property is %s", property);
        mFd = open(property, O_RDWR | O_NONBLOCK);
        //mFd = open("/dev/gsmtty19", O_RDWR);
	    ...
	    LOGD("mfd audiohal open mFd is %d", mFd); 
	    if (mFd < 0) {
		LOGE("Audio open property faild");
		return -EPERM;
	    }
	}
	...
	LOGD("mState = AUDIO_INPUT_OPENED");
...
ssize_t AudioHardware::AudioStreamInALSA::read(void* buffer, ssize_t bytes)
{
    ...
    LOGV("-----AudioStreamInALSA::read(%p, %d) mFormat is 0x%x START", buffer, (int)bytes, mFormat);
    //if ((mFormat != AudioSystem::AMR_NB)&&(mHardware->mode() != AudioSystem::MODE_IN_CALL)) {
	...
	{ // scope for the lock
		...
		if (spOut != 0) {
		    if (!spOut->checkStandby()) {
			LOGD("AudioStreamInALSA::read() force output standby");
			spOut->close_l();
 ...
 Error:
    ...
    LOGV("voice record only,other case don't enter!");
	    size_t count = bytes;
	    bytes = 0;
	    uint8_t* p = static_cast<uint8_t*>(buffer);
	    uint8_t* s = static_cast<uint8_t*>(buffer);
	    status_t status  = 0;

	    //LOGV("The count of voicememo  is %d", count);
	    //LOGV("The mState of voicememo  is %d", mState);

	    if (mState < AUDIO_INPUT_OPENED) {
		LOGV("AudioHardware::AudioStreamInALSA::read mState < AUDIO_INPUT_OPENED");
		AudioHardware *hw = mHardware;

		hw->mLock.lock();
		status = set(hw, mDevices, &mFormat, &mChannels, &mSampleRate, ((AudioSystem::audio_in_acoustics)0));
		hw->mLock.unlock();

		//LOGV("The status of voicememo set: %d", status);
		if (status != NO_ERROR) {
		    return -1;
		    LOGE("Error in set operation in setting operation");
		}
	    }

	    LOGV("The status of voicememo read set is: %d", status);

	    if (mState < AUDIO_INPUT_STARTED) {
		mState = AUDIO_INPUT_STARTED;
		LOGV("The voicememo read start, mState is %d", mState);
		//use ril tell the cp open
		LOGV("setCallVmOperation_l VOICE_RECORD_START");
		if (mHardware->setCallVmOperation_l(VOICE_RECORD_START)) {
		    LOGV("setCallVmOperation_l VOICE_RECORD_START Error ");
		    standby();
		    return -1;
		}
	    }
	    ...
	    bytes = 0;
	    while (count > 0) {
		//LOGD("read one time the count is %d",count);
		ssize_t bytesRead = 0;
		fd_set rfds;
		FD_ZERO(&rfds);
		FD_SET(mFd, &rfds);
		...
		int selectStatus = 0;
		selectStatus = ::select(mFd+1,&rfds,NULL,NULL,&timeout);
		if (selectStatus > 0) {
		    if(FD_ISSET(mFd, &rfds))
			bytesRead = ::read(mFd, p, count);
		} else
		    LOGE("selectStatus is %d read timeout!",selectStatus);
		...
		if (bytesRead > 0) {
		    ...
		    LOGV("count is = %d", count);
		} else if(bytesRead == 0) {
		    LOGV("Bytes Read = %d ,Mux Buffer is empty!",bytesRead);
		    continue;
		} else {
 	    ...
 	    LOGV("return  bytes is %d", bytes);

	    int i = 0;
	    for(i = 0; i<10;i++)
		LOGV("audiohardware the buffer is 0x%x",*(s+i));
...
status_t AudioHardware::AudioStreamInALSA::standby()
{
    ...
    if (mFormat == AudioSystem::AMR_NB) {
	LOGD("audiohal AudioStreamInALSA::close amr mState is %d", mState);
	if (mState > AUDIO_INPUT_CLOSED) {
	    mState = AUDIO_INPUT_CLOSED;
	    LOGV("audiohal AudioStreamInALSA::close mState > AUDIO_INPUT_CLOSED");
	    mHardware->setCallVmOperation_l(VOICE_RECORD_CLOSE);
	}
...
status_t AudioHardware::AudioStreamInALSA::open_l()
{
    ...
    LOGD("open pcm_in driver, mDevices(0x%x), mBtScoIn(%d)", mDevices, mBtScoIn);
    TRACE_DRIVER_IN(DRV_PCM_OPEN)
    ...
    // start bt sco pcm.
    if (mHardware->mode() != AudioSystem::MODE_IN_CALL && (!mBtScoIn) &&
       (mDevices & AudioSystem::DEVICE_IN_BLUETOOTH_SCO_HEADSET)) {
        LOGD("AudioStreamInALSA::open_l(): start bt sco related pcm");
        mHardware->openVoicePcmOut_l();
        mBtScoIn = true;
    }
        ...
        for(int cnt = 0 ; NULL != mixer[cnt].ctl ; ++cnt){
            ...
            if (NULL != mRouteCtl) {
                ...
                for (unsigned int idx = 0 ; mixer_ctl_get_num_values(mRouteCtl) > idx ; ++idx)
                    mixer_ctl_set_value(mRouteCtl, idx, mixer[cnt].val);
                TRACE_DRIVER_OUT
                LOGD("read() wakeup setting %s(%d) cnt:%d", mixer[cnt].ctl, mixer[cnt].val, cnt);
            } else {
                LOGE("111failed get mixer control");
            }
        }
    }
...
extern "C" int sendAudCalibrationAT(const char* atdata, int size, char** out) {
    int ret = -1;

    ret = SecRil::instance()->writeAndReadPhonetoolAT((void* )atdata, size, out);
    return ret;
}
